...
trait TestUtils {
  ...
  trait UsingTempDir extends After {
    protected lazy val tempDir = {
      val x = Files.createTempDirectory(Paths.get("/tmp"), getClass.getSimpleName+".")
      println("[%s]: created temp dir: %s".format(getClass.getSimpleName, x))
      x
    }
    def cleanup() {}
    def after = {
      ...
      println("[%s]: cleaned up temp dir: %s".format(getClass.getSimpleName, tempDir))
    }
  }
}
